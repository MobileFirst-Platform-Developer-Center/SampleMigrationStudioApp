...
/* JavaScript content from plugins/cordova-plugin-mfp/worklight/worklight.js in JS Resources */
/*
...
/**
  ...
  this.openURL = function(url,target,options){
    console.log("calling __openURL")
    this.__openURL(url,target,options);
  }
  ...
  this.getDeviceLanguage = function() {
    return this.__getDeviceLanguage();
  }
  ...
  // Back Button support (Work on Android && Windows Phone) ------ Not present in 8.0
    /**
...
__WLClient = function() {
	...
	this.obtainAccessToken = function(scope){
		var dfd = WLJQ.Deferred();
		WLAuthorizationManager.obtainAccessToken(scope)
		.then (
			function(accessToken) {
				 WL.Client.setConnected(true);
				 wlAccessTokens[scope] = accessToken.value;
				 wlLastAccessToken = accessToken.value;
	...
	this.getRequiredAccessTokenScope = function(status, headerValue) {
		var isAuthorizationRequired = WLAuthorizationManager.isAuthorizationRequired(status,headerValue);
		if(isAuthorizationRequired){
			return WLAuthorizationManager.getResourceScope(headerValue);
		} else{
			return null;
		}
	}
	...
	function onEnvInit(options) {
      ...
      WL.Logger.warn('Initialization option \'analytics\' is deprecated. Use WL.Analytics.enable/disable to set analytics data capture.');
		...
		if (WL.Client.getEnvironment() != WL.Env.MOBILE_WEB) {
			WLJSX.Ajax.WLRequest.setConnected = WL.Client.setConnected.bind(this);
		} else {
	...
	this.init = function(options) {
		...
		if ( typeof options.connectOnStartup === 'undefined' || options.connectOnStartup === null) {
			...
			console.warn('Initialization option \'connectOnStartup\' is deprecated. Use WL.Client.connect() to connect to the IBM MobileFirst Platform Server.');
		}
		...
		// Code for invoking push onReadyToSubscribe
		var firePush = function(){
			var checks = 0;
			var pushInterval = setInterval(checkPushReady, 3000);
			function checkPushReady() {
				checks++;
				if(WL.Client.Push.onReadyToSubscribe !== undefined){
		...
		var wlInit = function() {
			WL.Client.Push.initialize(function(){
				MFPush.registerNotificationsCallback(function(response){
					WL.Client.Push.onMessage({},response);
				})
				firePush();
			}, function() {
				console.error("Failure in initializing Push")
			})
...
//				    WL.Client.connect(connectOptions);
  ...
  this.connect = function(options) {
	...
	this.logActivity = function(activityType) {
		WL.Analytics.log("Activity [" + activityType + "] logged successfully.");
		WL.Logger.send();
	};
	 ...
	 this._defineProperty = function(obj, key, value) { if (key in obj) { Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true }); } else { obj[key] = value; } return obj; }
	...
	this.login = function(realm,options){
		...
		WL.Validators.validateOptions({
			...
			onFailure : 'function',
			parameters: 'object'
		}, options, 'WL.Client.login');
		options = extendWithDefaultOptions(options);
			var credentials = options.parameters;
			WLAuthorizationManager.login(realm,credentials).then(
					function(){
							userInfo[realm] = WL.Client._defineProperty({}, WL.UserInfo.IS_USER_AUTHENTICATED, true);
							if(credentials.username){
								userInfo[realm][WL.UserInfo.LOGIN_NAME] = credentials.username;
								userInfo[realm][WL.UserInfo.USER_NAME] = credentials.username;
							}
							options.onSuccess();
					}, function(failure){
	...
	this.logout = function(realm,options) {
		...
		WL.Validators.validateOptions({
			...
			onFailure : 'function',
			parameters: 'object'
		}, options, 'WL.Client.logout');
		options = extendWithDefaultOptions(options);

		WLAuthorizationManager.logout(realm).then(
			function(){
				if ( typeof userInfo[realm] === "undefined") {
	...
	this.isUserAuthenticated = function(realm) {
		console.warn("isUserAuthenticated API is no longer supported");
	...
	this.checkForDirectUpdate = function(options){
		WL.Client.login("directUpdate", options);
	}

	this.getUserPref = function(key) {
		console.warn("getUserPref API is no longer supported");
		return userPrefs[key] || null;
	};


	this.setUserPref = function(key, value, options) {
		console.warn("setUserPref API is no longer supported");
		options.onFailure("setUserPref API is no longer supported");
	}

	this.setUserPrefs = function(userPrefsHash, options) {
		console.warn("setUserPrefs API is no longer supported");
		options.onFailure("setUserPrefs API is no longer supported");
	}

	this.hasUserPref = function(key) {
		console.warn("hasUserPref API is no longer supported");
		return ( key in userPrefs);
	};

	this.deleteUserPref = function(key, options) {
		console.warn("deleteUserPref API is no longer supported");
		options.onFailure("deleteUserPref API is no longer supported");
	}

	this.updateUserInfo = function(options) {
		console.warn("updateUserInfo API is no longer supported");
		options.onFailure("updateUserInfo API is no longer supported");
	}

	this.getLoginName = function(realm) {
		console.warn("getLoginName API is no longer supported");
		var result = null;
...
this.pinTrustedCertificatePublicKey = function(certificateFilename){
	...
	this.__invokeProcedure = function(invocationData, options, useSendInvoke) {
		...
		function onInvokeProcedureFailure(transport) {
			if (!blocked) {
				blocked = true;
				var errorCode = "";
				if(transport.responseJSON && transport.responseJSON.errorCode){
				    errorCode = transport.responseJSON.errorCode;
				}
   ...
   /**
	...
	this.createChallengeHandler = function(securityCheck) {
		...
		if(WL.Client.getEnvironment() === WL.Env.ANDROID){
			...
			challengeHandler.submitLoginForm = function(reqURL,options,submitLoginFormCallback){
				...
				cordova.exec(null,null,"ChallengeHandlerPlugin","submitChallengeAnswer",[securityCheck,options.parameters]);
			}

			challengeHandler.submitAdapterAuthentication = function(invocationData, options) {
					challengeHandler.isAdapterAuth = true;
					cordova.exec(null,null,"ChallengeHandlerPlugin","submitChallengeAnswer",[securityCheck,options.parameters]);
			};
			...
			cordova.exec(function(pluginResult){
				...
				if(pluginResult.action == "handleChallenge"){
						...
						pluginResult.data.responseJSON = {
								authStatus : "credentialsRequired"
						}
						challengeHandler.handleChallenge(pluginResult.data);
				} else if(pluginResult.action == "handleSuccess"){
						console.log("isAdapter Auth : " + challengeHandler.isAdapterAuth);
						WL.Logger.debug("exec success: attempt to trigger submitSuccess");
						if(challengeHandler.isAdapterAuth){
								console.log("isAdapterAuth block")
			...
			function(pluginResult){
				console.log("Failure + " + JSON.stringify(pluginResult));
				WL.Logger.debug(JSON.stringify(pluginResult));
				if(pluginResult.action == "handleFailure"){
						...
						if(challengeHandler.isAdapterAuth){
								...
								challengeHandler.handleChallenge({
										responseJSON:{
												errorMessage: pluginResult.data.failure,
												authStatus : "credentialsRequired"
										}
								})
						}
		...
		return challengeHandler;
			...
			challengeHandler.submitLoginForm = function(reqUrl,answer,submitLoginFormCallback) {
					challengeHandler.submitLoginFormCallback = submitLoginFormCallback;
					if (( typeof answer === "undefined") || answer === null) {
							challengeHandler.activeRequest.removeExpectedAnswer(this.realm);
					} else {
							challengeHandler.activeRequest.submitAnswer(this.realm, answer.parameters);
					}
			...
			challengeHandler.handleSuccess = function(identity) {
				if(challengeHandler.isAdapterAuth){
					...
					challengeHandler.submitLoginFormCallback(identity);
				} 
			};
			...
			challengeHandler.handleFailure = function(err) {
				if(challengeHandler.isAdapterAuth){
					challengeHandler.handleChallenge({
							responseJSON:{
									errorMessage: err.failure,
									authStatus : "credentialsRequired"
							}
					})
				}
				else{
					challengeHandler.submitLoginFormCallback(err);
				}
			};
															 
			challengeHandler.submitAdapterAuthentication = function(invocationData, options) {
					challengeHandler.isAdapterAuth = true;
					challengeHandler.activeRequest.submitAnswer(this.realm, options.parameters);
					challengeHandler.activeRequest = null;
			};
	...
	this.getUsername = function() {
		WL.Logger.warn("getUsername API is no longer supported");
		var username = null;
...
__WLPush = function() {
  ...
  this.initialize = function(success, failure, options, timeout) {
    MFPush.initialize(success, failure, options, timeout);
  }

  this.getInteractivePushCategories = function() {
    WL.Logger.error("getInteractivePushCategories API is no longer supported");
  }

  this.subscribeSMS = function(alias, adapter, eventSource, phoneNumber, options) {
    var registerOptions = options;
    registerOptions['phoneNumber'] = phoneNumber;

    MFPush.registerDevice(options,function(success){
      MFPush.subscribe([alias], function(successResponse){
  ...
  this.backgroundJobDone = function(id,result) {
      MFPush.backgroundJobDone(id,result);
  }

  this.subscribeTag = function(tagName,options) {
    MFPush.registerDevice(options,function(success){
      MFPush.subscribe([tagName], function(successResponse){
  ...
  this.isPushSupported = function() {
    if(WL.Client.getEnvironment() === WL.Env.ANDROID || WL.Client.getEnvironment() === WL.Env.IPHONE){
			return true;
		}
    return false;
  };
  ...
  this.registerEventSourceCallback = function(alias, adapter, eventSource, callback){
    console.warn("registerEventSourceCallback is no longer supported. WL.Client.Push.onMessage is triggered when there is a notification")
  }
}
...
/**
 * ================================================================= 
 * Source file taken from :: MFPPush.js
 * ================================================================= 
 */
...
limitations under the License.
*/
window.MFPush = (function(){
  /**
	 * Initializes MFPush instance. This is required for the client application to connect to MFPush service with the right
	 * application context. This API should be called first before using other MFPush APIs.
	 *
	 * @param success callback
	 * @param failure callback
	 * @param timeout request timeout in seconds
	 */
var initialize = function(success, failure, options, timeout) {
 ...
 * @param failure callback
 */
var isPushSupported = function(success, failure) {
 ...
 * @param failure callback
 */
var getSubscriptions = function(success, failure) {
 ...
 * @param failure callback
 */
var getTags = function(success, failure) {
...
var subscribe = function(tags, success, failure) {
...
var unsubscribe = function(tags, success, failure) {
 ...
 * @param failure callback
 */
var registerDevice = function(options,success, failure) {
 ...
 * @param failure callback
 */
var unregisterDevice = function(success, failure) {
 ...
 */
var registerNotificationsCallback = function(callback) {
   ...
   */
  var backgroundJobDone = function(id, result) {
  ...
  return{
    ...
    registerNotificationsCallback: registerNotificationsCallback,
    subscribe:subscribe,
...
WL.Device.getFriendlyName = function(options) {
    ...
    WL.Validators.validateArguments(['object'], arguments, 'WL.Device.getFriendlyName');

    WL.Validators.validateOptions({
    	onSuccess : 'function',
    	onFailure : 'function'
    }, options, 'WL.Device.getFriendlyName');
...
WL.Device.setFriendlyName = function(friendlyName,options) {
    WL.Validators.validateArguments(['string', WL.Validators.validateObjectOrNull], arguments, 'WL.Device.setFriendlyName');

    if ( typeof options !== "undefined") {
    	WL.Validators.validateOptions({
            onSuccess : 'function',
    		onFailure : 'function'
    	}, options, 'WL.Device.setFriendlyName');
    } else {
    	options = {};
    	options.onSuccess = function(){};
    	options.onFailure = function(){};
    ...
    WL.DeviceAuth.__setDeviceDisplayName(friendlyName, options.onSuccess, options.onFailure);
}
...
WL.Device.getID = function(options) {
    ...
    WL.Validators.validateOptions({
    	onSuccess : 'function',
    	onFailure : 'function'
    }, options, 'WL.Device.getID');

    WL.DeviceAuth.__getDeviceUUID(options.onSuccess,options.onFailure);
}
...
WL.Toast.show = function(text) {
    console.warn("Toast is not supported on iOS Platform");
};
...
WL.App.close = function() {
    ...
    console.warn("Close API is not supported on iOS")
};
...
window.WLAuthorizationManager = (function () {
  ...
  var getAuthorizationScope = function (responseAuthenticationHeader) {
    return WLAuthorizationManager.__getAuthorizationScope(responseAuthenticationHeader);
  }
...
var setLoginTimeoutNonAndroid = function (timeout)
{
 ...
 function sortScopeAlphabetically(scope) {
  ...
  function obtainAuthorizationHeader(scope){
    var dfd = WLJQ.Deferred();
    ...
    WLAuthorizationManager.__invokeAuthorizationRequestForScope(scope).then(
       function(accessToken){
          dfd.resolve(accessToken.asAuthorizationRequestHeader);
       }, function(err){
          dfd.reject(err);
       }
    )
  ...
  function getCachedAuthorizationHeader(){
    var dfd = WLJQ.Deferred();
    cordova.exec(
      function(data) {
        dfd.resolve(data);
  ...
  var getAppIdentity = function() {
    console.warn("The API getAppIdentity is no longer supported");
    if(WL.Client.getEnvironment() === WL.Env.ANDROID){
      var dfd = WLJQ.Deferred();
      cordova.exec(
        function(data) {
      ...
      var dfd = WLJQ.Deferred();
      return dfd.reject("The API getAppIdentity is no longer supported");
    }
  };

  var getDeviceIdentity = function() {
    console.warn("The API getDeviceIdentity is no longer supported");
    if(WL.Client.getEnvironment() === WL.Env.ANDROID){
      var dfd = WLJQ.Deferred();
      cordova.exec(
        function(data) {
      ...
      var dfd = WLJQ.Deferred();
      return dfd.reject("The API getDeviceIdentity is no longer supported");
    }
  };

  var getUserIdentity = function() {
    console.warn("The API getUserIdentity is no longer supported");
    var dfd = WLJQ.Deferred();
    return dfd.reject("The API getUserIdentity is no longer supported");
  }
...
WL.BusyIndicator = WLJSX.Class.create({
    ...
    show : function() {
    	console.warn("Busy Indicator is not supported on iOS Platform");
    },
